...
class OpenEmbedder:
    ...
    def _initialize_vectordb(self):
        ...
        if self.vectordb_name == "chromadb":
            ...
            try:
                ...
                if self.collection_name not in [col.name for col in existing_collections]:
                    ...
                    client.create_collection(
                        ...
                        vectors_config=models.VectorParams(
                            size=self._get_embedding_size(), distance=models.Distance.COSINE
                        ),
                    )
                ...
                raise ValueError(
                    f"Please make sure qdrant db is running on port 6333: {str(e)}") from e

        elif self.vectordb_name == "faiss":
            faiss_index_file = os.path.join(self.persist_directory,
                                            f"{self.collection_name}_faiss.index.faiss")
            vectordb = None
            if os.path.exists(faiss_index_file):
                # Load the existing FAISS index
                vectordb = FAISS.load_local(folder_path=self.persist_directory,
    ...
    def create_database(self, text_file_path: str):
        ...
        if self.vectordb_name == "faiss":
            faiss_index_file = os.path.join(self.persist_directory,
                                            f"{self.collection_name}_faiss.index")
            index_to_docstore_id = {}  # Initialize an empty mapping for the document IDs
            vectordb = FAISS(
                embedding_function=self.embeddings,
                index=self.index,
